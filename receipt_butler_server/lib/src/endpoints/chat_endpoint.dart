  ...
  Future<String> ask(Session session, String question) async {
    try {
      ...
      final receipts = await Receipt.db.find(
        session,
        limit:
            200, // Increased to 500: Gemini Flash has a huge context window (1M tokens).
      ...
      final contextData = receipts
          .map((r) {
            String details = "${r.merchantName}";
            if (r.senderName != null || r.recipientName != null) {
              details +=
                  " | From: ${r.senderName ?? 'N/A'} -> To: ${r.recipientName ?? 'N/A'}";
            }
...
""";
      ...
      var apiKey =
          session.serverpod.getPassword('OPENROUTER_KEY') ?? 'sk-or-v1-...';
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode({
              'model':
                  'deepseek/deepseek-r1-0528:free', // User preferred free model
              'max_tokens': 5000,
              'messages': [
